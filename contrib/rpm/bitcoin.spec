...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19091
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19090
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 39091
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 39090
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 19091
	%{_sbindir}/semanage port -d -p tcp 19090
	%{_sbindir}/semanage port -d -p tcp 39091
	%{_sbindir}/semanage port -d -p tcp 39090
	for selinuxvariant in %{selinux_variants}; do
