...
#if defined(NDEBUG)
# error "Rocketcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Rocketcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s RCKT additional fees, %d delta bytes\n",
...
bool IsInitialBlockDownload()
{
    ...
    if (latchToFalse.load(std::memory_order_relaxed))
    {
        //LogPrintf("Memory - InitialBlockDownload \n");
        return false;
    }
    if (fImporting || fReindex)
    {
       //LogPrintf("Importing - InitialBlockDownload \n");
 
        return true;
    }
    if (chainActive.Tip() == nullptr)
    {
        //LogPrintf("Nul Tip - InitialBlockDownload \n");
 
        return true;
    }
    if (chainActive.Tip()->nChainWork < UintToArith256(chainParams.GetConsensus().nMinimumChainWork))
    {  
        //LogPrintf("Chain Work - InitialBlockDownload \n");
        return true;
    }
    if (chainActive.Tip()->GetBlockTime() < (GetTime() - nMaxTipAge))
    {
        //LogPrintf("Block Time - InitialBlockDownload \n");
        return true;
    }
