...
Run with the -testnet option to run with "play rocketcoins" on the test network, if you
...
Rocketcoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 19090.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 19091 for connections and services them.

- Rocketcoin Miner : Generates rocketcoins (if wallet is enabled).
...
However, in open source software such as Rocketcoin Core, where everyone uses
...
pay attention to for reviewers of Rocketcoin Core code.

General Rocketcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Rocketcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Rocketcoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Rocketcoin Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:rocketcoin-project/rocketcoin.git
